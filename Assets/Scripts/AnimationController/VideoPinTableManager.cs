...
{
    [System.Serializable]
    public class VideoPinTableData
    {
        ...
        public VideoPinTableData(int[,] pinTable, Color[,] colorMax, Color[,] colorMin, int baseRow, int baseCol, int row, int col)
        {
    ...
    public class VideoPinTableManager : MonoBehaviour
    {
        public VideoPinTableGenerator pinTableGenerator;
        ...
        public VideoPinTableManager(int baseRow, int baseCol, int row, int col)
        {
            this.row = row;
            this.col = col;
            this.baseRow = baseRow;
        ...
        public void InitializePinTable()
        {
            ...
            pinTableGenerator.SetHeights(PinTable, 0);
        }

        #region Generator for JSONs & Packages

        public IEnumerator GenerateJSONs(string serializedData)
        {
        ...
        public IEnumerator GeneratePackages(string serializedData)
        {
            yield return GenerateJSONs(serializedData);
            int packageIndex = 0;
        ...
        private string SerializeData()
        {
            ...
            VideoPinTableData data = new VideoPinTableData(PinTable, colorMax, colorMin, baseRow, baseCol, row, col);
        ...
        public IEnumerator LoadDataFromFile(string path, float waitTime)
        {
            if (File.Exists(path))
            {
                ...
                yield return DeserializeData(serializedData, waitTime);
            }
            else
            {
        ...
        public bool AllHeightTransitionsFinished()
        {
            foreach (var pin in pinTableGenerator.PinTable)
            {
                if (pin != null && pin.IsTransitioning())
                {
                    return false;
                }
            }
        ...
        private IEnumerator DeserializeData(string serializedData, float waitTime)
        {
            print(serializedData);
            var deserializedData = JsonUtility.FromJson<VideoPinTableData>(serializedData);
            ...
            if (deserializedData.serializedColorMax != null && deserializedData.serializedColorMin != null)
            {
                ...
                pinTableGenerator.SetHeights(PinTable, waitTime);
        ...
        public static implicit operator VideoPinTableManager(PinTableConnector v)
        {
