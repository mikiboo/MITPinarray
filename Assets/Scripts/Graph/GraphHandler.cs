...
public class GraphHandler : MonoBehaviour
{
    ...
    #endregion
    public void UpdateRandomPointY(int index)
    {
        if (index >= 0 && index < values.Count)
        {
            float randomY = UnityEngine.Random.Range(-10.0f, 10.0f); // Use UnityEngine's Random
            Vector2 newValue = new Vector2(values[index].x, randomY);
            ChangePoint(index, newValue);
        }
    }
    #region references
    ...
    private void Update()
    {
        ...
        if (values.Count > 0)
        {
            UpdateRandomPointY(0); // Randomly updates the Y-value of the first point every frame
        }
        //for (int i = 0; i <= xAxisRange.y + 1; i += 2)
        //{
        //    Debug.Log(xAxisRange.y + 1);
